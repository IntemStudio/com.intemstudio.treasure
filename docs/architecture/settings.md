...
| 진입 | MenuShell 플레이어 메뉴 Settings 탭, 타이틀 SettingsHost |
...
│   ├── LeftColumn (Panel) → 탭별 패널(행)
│   └── RightPanel → DetailTitle · DetailBody
└── ExitPanel                          # 나가기 전용
    ├── ExitLeftPanel → ExitList
    └── ExitRightPanel → 제목 · 본문 · 예/아니요
```
...
인게임 Q/E는 TopBar 플레이어 탭. 푸터: BACK
